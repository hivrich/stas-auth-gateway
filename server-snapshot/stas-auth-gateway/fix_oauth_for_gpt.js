...
function fixOAuthTokenEndpoint(app) {
  ...
  app.post('/gw/oauth/token', async (req, res) => {
    ...
    console.log('OAuth token request headers:', req.headers);

    const { grant_type, code, redirect_uri, client_id, client_secret } = req.body || {};

    console.log('Parsed:', { grant_type, code, redirect_uri, client_id });
    ...
    try {
      ...
      if (redirect_uri) {
        try {
          ...
          console.log('Found user_id in redirect_uri:', user_id);
        } catch (e) {
      ...
      if (!user_id && code) {
        ...
        try {
          ...
          if (payload.typ === 'auth_code' && payload.sub) {
            ...
            console.log('Found user_id in JWT code:', user_id);
          }
        } catch (e) {
