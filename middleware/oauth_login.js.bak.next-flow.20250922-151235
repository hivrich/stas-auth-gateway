...
module.exports = function oauthLogin() {
  ...
  // Готовим токен
  const token = 't_' + Buffer.from(JSON.stringify({ uid: cleaned })).toString('base64')
  ...
  // Прокинем дальше как ждал старый код
  if (!req.headers.authorization) req.headers.authorization = 'Bearer ' + token;
  if (req.body) { req.body.user_id = cleaned; req.body.uid = cleaned; }

  // НЕ делаем redirect здесь — отдаём управление родному обработчику
  return next();
});
res.cookie('STAS_UID', cleaned,   { httpOnly:true, sameSite:'lax', path:'/gw', maxAge:7*24*60*60*1000 });
