...
function parseBearerUid(req) {
  ...
  let obj;
  try {
    obj = JSON.parse(Buffer.from(b64 + pad, 'base64').toString('utf8'));
  } catch (e) {
    throw Object.assign(new Error('invalid_token_payload'), { status: 401 });
  }
...
module.exports = function attachIcuPostDryRun(app) {
  ...
  router.post('/events', express.json({ limit: '256kb' }), async (req, res) => {
    try {
      const uid = parseBearerUid(req); // eslint-disable-line no-unused-vars
