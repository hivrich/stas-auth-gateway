...
function tokenIngress(req, res, next) {
  ...
  // Keep the internal request_id for existing consumers; stas_request_id is
  // the caller-supplied id accepted by the request-id ingress middleware.
  const common = { request_id: randomUUID(), stas_request_id: getStasRequestId(req) || null,
    method: 'POST', path: '/gw/oauth/token',
