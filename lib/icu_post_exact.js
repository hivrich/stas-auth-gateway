  ...
  app.post('/gw/icu/events', express.json({ limit: '256kb' }), async (req, res) => {
    try {
      ...
      if (dedupe) {
        ...
        const { apiKey, athleteId } = await getCreds2(uid);
        const existing = await icuListEvents({ apiKey, athleteId, q: { external_id_prefix: prefix, oldest, newest } });
      ...
      const icuResp = await icuCreateEvents({ apiKey, athleteId, events: normalized });
