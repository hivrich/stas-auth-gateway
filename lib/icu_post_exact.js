...
const { icuCreateEvents } = require('./icu_write');
const { icuListEvents } = require('./icu_delete');
const { getIcuRequestAuth } = require('./icu-request-auth');
const { getRequestUserId } = require('./request-auth');
const { applyInferredWorkoutTarget } = require('./structured_workout_target');
...
function normalizeEvents(events, prefix = 'plan:') {
  return events.map(ev => {
    ...
    return applyInferredWorkoutTarget(e);
  });
}
  ...
  app.post('/gw/icu/events', express.json({ limit: '256kb' }), async (req, res) => {
    try {
      ...
      if (dedupe) {
        ...
        const existing = await icuListEvents({
          apiKey: auth.token,
          athlete_id: auth.athleteId,
          q: { external_id_prefix: prefix, oldest, newest },
      ...
      const icuResp = await icuCreateEvents({
        apiKey: auth.token,
        athlete_id: auth.athleteId,
        events: normalized,
        allowBasicFallback: auth.authMode !== 'intervals',
