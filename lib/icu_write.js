...
async function icuCreateEvents({ apiKey, athlete_id, events, allowBasicFallback = true }) {
  ...
  for (const e of events) {
    ...
    let resp;
    try {
      resp = await postJSON(url, { headers: { Authorization: `Bearer ${apiKey}` }, body: e });
    } catch (_err) {
      if (!allowBasicFallback) throw _err;
      const auth = 'Basic ' + b64(`API_KEY:${apiKey}`);
      resp = await postJSON(url, { headers: { Authorization: auth }, body: e });
    }
    results.push(resp);
  }
  return results;
