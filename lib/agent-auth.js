...
const PLACEHOLDER_AGENT_AUTH_SECRETS = new Set([
  'agent-auth-disabled',
  'changeme',
  'change-me',
  'change_me',
...
const PLACEHOLDER_AGENT_AUTH_SECRET_MARKERS = [
  'change-me',
  'changeme',
  'generate-with',
  'openssl',
...
function isUsableAgentAuthSecret(secret) {
  const raw = trimToString(secret);
  const normalized = raw.toLowerCase();
  return raw.length >= 32
    && !PLACEHOLDER_AGENT_AUTH_SECRETS.has(normalized)
    && !PLACEHOLDER_AGENT_AUTH_SECRET_MARKERS.some((marker) => normalized.includes(marker));
}

function isAgentAuthConfigured() {
  return boolFromEnv(process.env.AGENT_AUTH_ENABLED) && isUsableAgentAuthSecret(getAgentAuthSecret());
}
...
function buildClaimResponse(registration, claimToken, origin, userCode) {
  ...
  return {
    ...
    claim: {
      ...
      verification_uri_complete: userCode
        ? `${buildVerificationUri(origin)}?user_code=${encodeURIComponent(userCode)}`
        : null,
      expires_in: expiresIn,
      interval,
    },
