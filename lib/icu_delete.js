...
async function getIcuCredsForUid(uid) {
  ...
  const creds = await httpJSON(url, { headers: { 'X-API-Key': key, "X-API-Key": getStasKey(), "Accept": "application/json" } });
...
async function icuListEvents({ apiKey, athlete_id, q = {} }) {
  ...
  return await httpJSON(u.toString(), { headers: { Authorization: auth, Accept: 'application/json', "X-API-Key": getStasKey(), "Accept": "application/json" } });
}
...
async function icuDeleteEventById({ apiKey, athlete_id, id }) {
  const auth = 'Basic ' + b64(`API_KEY:${apiKey}`);
  const url  = `https://intervals.icu/api/v1/athlete/${encodeURIComponent(athlete_id)}/events/${encodeURIComponent(id)}`;
  return await httpDelete(url, { headers: { Authorization: auth, Accept:'application/json', "X-API-Key": getStasKey(), "Accept": "application/json" } });
}
