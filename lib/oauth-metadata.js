...
const { MCP_SCOPES_SUPPORTED } = require('./mcp-oauth-scopes');
...
function buildOAuthAuthorizationServerMetadata(origin, env = process.env) {
  ...
  if (env.INTERVALS_CLIENT_ID && env.INTERVALS_CLIENT_SECRET) {
    methods.push('none', 'private_key_jwt');
  }
  ...
  const metadata = {
    ...
    ...(methods.includes('private_key_jwt') ? { token_endpoint_auth_signing_alg_values_supported: ['RS256'] } : {}),
  ...
  if (isAgentAuthConfigured()) {
    metadata.grant_types_supported = ['authorization_code', 'refresh_token', AGENT_AUTH_GRANT_TYPE];
