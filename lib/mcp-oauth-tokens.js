...
function createTokenRecord(params) {
  ...
  const common = {
    ...
    source: 'mcp',
    ...(params.clientAuthMethod === 'private_key_jwt' ? { cam: 'private_key_jwt' } : {}),
    resource,
    scope: scopes.join(' '),
...
class PgTokenStore {
  ...
  async consumeClientAssertion(hash, expiresAt, now = new Date()) {
    // One unique insert arbitrates across processes/restarts. Cleanup work is
    // bounded per authenticated attempt; only short-lived hashes are retained.
    const result = await this.getPool().query(
      `WITH expired AS (
         SELECT assertion_hash FROM gw_oauth_client_assertions
         WHERE expires_at < $3 ORDER BY expires_at LIMIT 100
         FOR UPDATE SKIP LOCKED
       ), cleaned AS (
         DELETE FROM gw_oauth_client_assertions WHERE assertion_hash IN (SELECT assertion_hash FROM expired)
       )
       INSERT INTO gw_oauth_client_assertions (assertion_hash, expires_at)
       VALUES ($1, $2) ON CONFLICT (assertion_hash) DO NOTHING
...
class MemoryTokenStore {
  constructor() { this.records = []; this.assertions = new Map(); }
  async consumeClientAssertion(hash, expiresAt, now = new Date()) {
    for (const [key, expiry] of this.assertions) {
      if (expiry < now.getTime()) this.assertions.delete(key);
    }
    if (this.assertions.has(hash)) return false;
    this.assertions.set(hash, expiresAt.getTime());
    return true;
  }
...
function getMcpTokenClientId(token) {
  return getMcpTokenClient(token)?.clientId || null;
}

function getMcpTokenClient(token) {
  const raw = trimToString(token);
  const prefix = raw.startsWith(ACCESS_TOKEN_PREFIX)
    ? ACCESS_TOKEN_PREFIX
    : (raw.startsWith(REFRESH_TOKEN_PREFIX) ? REFRESH_TOKEN_PREFIX : '');
  if (!prefix) return null;
  const payload = verifyToken(raw, prefix);
  if (!payload || payload.source !== 'mcp') return null;
  const clientId = trimToString(payload.client_id);
  return clientId ? { clientId, tokenEndpointAuthMethod: payload.cam || 'none' } : null;
}

async function consumeMcpClientAssertion(hash, expiresAt, now) {
  return getStore().consumeClientAssertion(hash, expiresAt, now);
}
