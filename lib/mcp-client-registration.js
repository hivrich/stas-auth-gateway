...
function parseRedirectUri(value) {
  ...
  try {
    ...
    if (url.protocol === 'http:' && isLoopbackHostname(url.hostname) && url.port !== '0') {
...
function redirectUriMatches(registeredUri, requestedUri, applicationType) {
  ...
  // RFC 8252 section 8.4 requires an exact loopback redirect match except
  // for the ephemeral port. The hostname, path and query remain bound.
...
function readClientMetadata(body, options = {}) {
  ...
  // Client metadata declares client capabilities, not server obligations. Keep
  // only grants this server implements while requiring authorization_code.
  const supportedGrantTypes = uniqueGrantTypes.filter((grant) => SUPPORTED_GRANT_TYPES.has(grant));
  ...
  if (!SUPPORTED_TOKEN_ENDPOINT_AUTH_METHODS.has(tokenEndpointAuthMethod)) {
    return invalid('token_endpoint_auth_method_unsupported');
  }
  if (tokenEndpointAuthMethod !== 'none' && applicationType !== 'web') {
    return invalid('confidential_native_client_not_allowed');
  }
  // Fetched documents can publish public keys, but cannot provision a secret.
  if (options.expectedClientId && ['client_secret_basic', 'client_secret_post'].includes(tokenEndpointAuthMethod)) {
    return invalid('cimd_confidential_client_not_allowed');
  }
  let signingMetadata = {};
  if (tokenEndpointAuthMethod === 'private_key_jwt') {
    // Signed DCR IDs have a small bounded payload. Key-based registration is
    // currently supported through CIMD only; never provision a dummy secret.
    if (!options.expectedClientId) return invalid('private_key_jwt_requires_cimd');
    const alg = body.token_endpoint_auth_signing_alg ?? 'RS256';
    if (alg !== 'RS256') return invalid('signing_alg_unsupported');
    if ((body.jwks === undefined) === (body.jwks_uri === undefined)) return invalid('jwks_source_required');
    if (body.jwks_uri !== undefined) {
      const url = typeof body.jwks_uri === 'string' && parsePublicJsonUrl(body.jwks_uri);
      if (!url) return invalid('jwks_uri_invalid');
      signingMetadata = { jwksUri: url.toString(), tokenEndpointAuthSigningAlg: alg };
    } else {
      const jwks = normalizePublicJwks(body.jwks);
      if (!jwks) return invalid('jwks_invalid');
      signingMetadata = { jwks, tokenEndpointAuthSigningAlg: alg };
    }
  }
...
function parsePublicJsonUrl(value) {
  const raw = trimToString(value);
...
function parseCimdClientId(value) {
  const url = parsePublicJsonUrl(value);
  return url && url.pathname !== '/' ? url : null;
}

function normalizePublicJwks(body) {
  if (!body || !Array.isArray(body.keys) || body.keys.length < 1 || body.keys.length > 8) return null;
  const kids = new Set();
  const keys = [];
  for (const key of body.keys) {
    if (!key || typeof key !== 'object' || Array.isArray(key) || key.kty !== 'RSA') return null;
    if (['d', 'p', 'q', 'dp', 'dq', 'qi', 'oth', 'k'].some((name) => Object.hasOwn(key, name))) return null;
    if ((key.alg !== undefined && key.alg !== 'RS256') || (key.use !== undefined && key.use !== 'sig')) return null;
    if (key.key_ops !== undefined && (!Array.isArray(key.key_ops) || key.key_ops.length !== 1 || key.key_ops[0] !== 'verify')) return null;
    if (key.kid !== undefined && (typeof key.kid !== 'string' || !key.kid || key.kid.length > 256 || kids.has(key.kid))) return null;
    if (body.keys.length > 1 && key.kid === undefined) return null;
    if (key.kid !== undefined) kids.add(key.kid);
    if (typeof key.n !== 'string' || typeof key.e !== 'string' || !/^[A-Za-z0-9_-]{342,683}$/.test(key.n) || !/^[A-Za-z0-9_-]{1,8}$/.test(key.e)) return null;
    const normalized = { kty: 'RSA', n: key.n, e: key.e, ...(key.kid !== undefined ? { kid: key.kid } : {}), alg: 'RS256', use: 'sig' };
    try {
      const publicKey = crypto.createPublicKey({ key: normalized, format: 'jwk' });
      const bits = publicKey.asymmetricKeyDetails.modulusLength;
      if (bits < 2048 || bits > 4096 || publicKey.asymmetricKeyDetails.publicExponent < 65537n) return null;
    } catch { return null; }
...
async function fetchPublicJson(url, options = cimdOptionsOverride || {}, contentTypes = ['application/json']) {
  const lookup = options.lookup || dns.lookup;
  const fetchImpl = options.fetchImpl || nodeFetch;
  const signal = AbortSignal.timeout(CIMD_TIMEOUT_MS);
  let agent;
  try {
    // Include DNS in the deadline; the connection then uses only these
    // validated addresses, preserving TLS hostname verification.
    const addresses = await new Promise((resolve, reject) => {
      const abort = () => reject(new Error('cimd_fetch_failed'));
    ...
    agent = new https.Agent({
      ...
      lookup(_hostname, lookupOptions, callback) {
        const optionsValue = typeof lookupOptions === 'number'
          ? { family: lookupOptions }
          : (lookupOptions || {});
        const requestedFamily = Number(optionsValue.family || 0);
        const candidates = requestedFamily === 4 || requestedFamily === 6
          ? pinnedAddresses.filter((entry) => entry.family === requestedFamily)
          : pinnedAddresses;
    ...
    if (!contentTypes.includes(contentType.split(';')[0].trim())) return invalid('cimd_content_type_invalid', 'invalid_client');
...
async function loadCimdClientMetadata(clientId, options = cimdOptionsOverride || {}) {
  ...
  const result = readClientMetadata(fetched.body, { expectedClientId: url.toString() });
...
function summarizeClientMetadataInput(body, result) {
  ...
  const knownKeys = new Set([
    ...
    'response_types', 'token_endpoint_auth_method', 'jwks', 'jwks_uri', 'token_endpoint_auth_signing_alg',
  ]);
  ...
  return {
    ...
    grantTypes: summarizeEnumArray(body?.grant_types, SUPPORTED_GRANT_TYPES),
    responseTypes: summarizeEnumArray(body?.response_types, new Set(['code'])),
    tokenEndpointAuthMethod: SUPPORTED_TOKEN_ENDPOINT_AUTH_METHODS.has(trimToString(body?.token_endpoint_auth_method))
      ? trimToString(body.token_endpoint_auth_method)
