...
function readClientMetadata(body, options = {}) {
  ...
  // Client ID Metadata Documents are public client declarations. A secret
  // cannot be provisioned through a fetched document, so confidential methods
  // are only valid for clients created at the registration endpoint.
  if (options.expectedClientId && tokenEndpointAuthMethod !== 'none') {
